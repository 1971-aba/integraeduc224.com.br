...
export default async function GestorMerendaCardapiosPage() {
  const { profile } = await requireRole(["gestor_escolar", "admin_sme"]);

  if (!profile.escola_id) {
    return (
      <>
        <GestorPageHeader title="Cardápios da Escola" />
        <SemEscolaAlert />
      </>
    );
  }
  ...
  return (
    <>
      <GestorPageHeader
        title="Cardápios da Escola"
        description="Registro de cardápios servidos e alunos atendidos"
      />
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_320px]">
        <Card>
          <CardTitle>Histórico de cardápios</CardTitle>
          <CardDescription>
