import { listAlmoxarifadoItens } from "@/actions/gestor-estrutura-almoxarifado";
...
export default async function GestorAlmoxarifadoPage() {
  const { profile } = await requireRole(["gestor_escolar", "admin_sme"]);

  if (!profile.escola_id) {
    return (
      <>
        <GestorPageHeader title="Controle de Almoxarifado" />
  ...
  const estoqueBaixo = itens.filter((i) => i.quantidade <= i.estoqueMinimo).length;
  ...
  return (
    <>
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_300px]">
        ...
        <Card><AlmoxarifadoItemForm /></Card>
      </div>
    </>
  );
}
