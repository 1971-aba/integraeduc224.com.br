...
type FrequenciaConsolidadaViewProps = {
...
export function FrequenciaConsolidadaView({
  resumos,
  bimestres,
  bimestreId,
  filtro,
  basePath,
}: FrequenciaConsolidadaViewProps) {
  const filtroAtivo = Boolean(filtro.tipo);
  const clearHref = bimestreId
    ? `${basePath}?bimestre=${bimestreId}`
    : basePath;
      ...
      {resumos.length === 0 ? (
        <div className="rounded-lg border border-dashed border-slate-300 bg-slate-50 px-6 py-12 text-center text-sm text-slate-600">
          {filtroAtivo
            ...
            : "Nenhuma turma com chamadas registradas no período."}
        </div>
      ) : (
